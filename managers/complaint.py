...
class ComplaintManager:
    ...
    @staticmethod
    def get_issue_transaction(amount, full_name, iban, complaint_id):

        quote_id=wise.create_quote(amount)
        recipient_id=wise.create_recepient(full_name, iban)
    ...
    def create(complaint_data, complainer):
        ...
        """
        This function decodes the base 64 encoded string and upload the photo to AWS service.
        Commit to the database with a newly created complaint
        """
        ...
        ComplaintManager.get_issue_transaction(amount, full_name, iban, complaint.id)
    ...
    def update(complaint_data, id_):
        ...
        complaint=ComplaintModel.query.filter_by(id=id_).first()
        if not complaint:
